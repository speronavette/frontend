    ...
    const renderPrice = () => {
        try {
            ...
            if (booking.isRoundTrip) {
                const outboundPrice = calculatePrice(booking.outbound);
                const inboundPrice = calculatePrice(booking.inbound);
    ...
    useEffect(() => {
        const fetchBookingDetails = async () => {
            try {
                ...
                console.log("Données reçues:", location.state); // Ajout pour déboguer
    
                const { isRoundTrip, outboundBooking, inboundBooking, singleBooking, email } = location.state;
    
                // Pour un aller-retour
                if (isRoundTrip) {
                    if (outboundBooking && inboundBooking) {
                        // Structure pour deux bookings séparés
                        const bookingData = {
                    ...
                    } else if (singleBooking && singleBooking.journey?.type === 'roundTrip') {
                        // Adapter une structure single booking à la structure attendue par le composant
                        const bookingData = {
                            outbound: {
                                ...singleBooking,
                                journey: { outbound: singleBooking.journey.outbound }
                            },
                            inbound: {
                                ...singleBooking,
                                journey: { inbound: singleBooking.journey.inbound }
                            },
                            isRoundTrip: true
                        };
                        setBooking(bookingData);
                    } else {
                        throw new Error('Structure de données incorrecte pour un aller-retour');
                    }
                    return;
                }
