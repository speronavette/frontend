...
function Confirmation() {
    ...
    // Envoi de l'achat à GTM via dataLayer (une seule fois)
useEffect(() => {
  if (!booking) return;

  // Type de réservation / service
  ...
  // Valeur (mets la vraie si tu préfères; ici 12 comme avant)
  const value = 12;
  const currency = 'EUR';
  const transactionId = `RES-${Date.now()}`;

  window.dataLayer = window.dataLayer || [];
  window.dataLayer.push({
    event: 'purchase',                 // <- GTM écoutera cet event
    transaction_id: transactionId,     // utile pour Ads + GA4
    value,
    currency,
...
<>
  <SEO
                title="Confirmation de Réservation | Navette Aéroport Confirmée | Spero Navette"
