...
function Confirmation() {
            ...
            try {
                if (!location.state) {
                    ...
                    if (window.gtag) {
                        ...
                        // Également envoyer l'événement reservation_confirmee pour GA4
                        window.gtag('event', 'reservation_confirmee', {
                            transaction_id: 'DIRECT-' + Date.now().toString(),
                            value: 100,
                            currency: 'EUR',
                            service_type: 'direct_page',
                            booking_type: 'direct'
                        });
    ...
    // ✅ TRACKING DE CONVERSION PRINCIPAL - Google Analytics 4 + Google Ads
    useEffect(() => {
        if (booking && window.gtag) {
            ...
            // ✅ CALCULER LE VRAI PRIX TOTAL (au lieu de la valeur fixe de 12€)
            let totalPrice = 0;
            ...
            // ✅ Événement GA4 "reservation_confirmee" (lié à Google Ads via l'import GA4)
            window.gtag('event', 'reservation_confirmee', {
                'transaction_id': transactionId,
                'value': totalPrice,
                'currency': 'EUR',
                'service_type': serviceType === 'private' ? 'Navette Privée' : 'Navette Partagée',
                'booking_type': bookingType,
                'airports': airports.join(','),
                'passengers': booking.isRoundTrip ? booking.outbound.passengers : booking.passengers
            });
    ...
    // Suivi de conversion de secours (au cas où le tracking principal échoue)
    useEffect(() => {
        const sendBackupConversion = () => {
            if (window.gtag) {
                if (document.readyState === 'complete') {
                    ...
                    // Backup pour reservation_confirmee aussi
                    window.gtag('event', 'reservation_confirmee', {
                        'transaction_id': 'BACKUP-' + Date.now().toString(),
                        'value': 100,
                        'currency': 'EUR'
