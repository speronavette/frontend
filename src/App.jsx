...
// Lazy loading des composants de page - Utiliser des chunks plus précis
...
function App() {
  // Prechargement des pages associées
  const prefetchPage = (path) => {
    if (typeof import.meta !== 'undefined') {
      const page = path === '/' ? 'Home' : path.slice(1).charAt(0).toUpperCase() + path.slice(2);
      try {
        ...
        <nav className="bg-spero text-white p-4" aria-label="Navigation principale">
          <div className="max-w-6xl mx-auto flex justify-between items-center px-4">
            <div className="flex space-x-4">
              <Link to="/" className="hover:text-opacity-80" onMouseOver={() => handleMouseOver('/')}>Accueil</Link>
              <Link to="/services" className="hover:text-opacity-80" onMouseOver={() => handleMouseOver('/services')}>Services</Link>
              <Link to="/contact" className="hover:text-opacity-80" onMouseOver={() => handleMouseOver('/contact')}>Contactez-nous</Link>
              <Link to="/faq" className="hover:text-opacity-80" onMouseOver={() => handleMouseOver('/faq')}>FAQ</Link>
            </div>
          </div>
        </nav>
