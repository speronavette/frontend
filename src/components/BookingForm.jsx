...
const validateVatNumber = (vatNumber) => {
 ...
 const handleAdditionalInfoChange = (e) => {
   ...
   setAdditionalInfo(prev => ({ ...prev, [name]: value }));
       // Validation spécifique pour le numéro de TVA
       if (name === 'vatNumber') {
         const isValid = validateVatNumber(value);
         setFormErrors(prev => ({ 
           ...prev, 
           vatNumber: isValid ? undefined : 'Format invalide. Exemple: BE0123456789' 
         }));
       }
 };
...
navigate('/confirmation', {
  state: {
    outboundBooking: {
      ...data.data,
      journey: {
        outbound: data.data.journey.outbound
      }
    },
    inboundBooking: {
      ...data.data,
      journey: {
        inbound: data.data.journey.inbound
      }
    },
    email: personalInfo.email,
    isRoundTrip: true
           ...
           {(journeyType === 'inbound' || journeyType === 'roundTrip') && (
             <div className="mt-6 p-6 border-2 border-spero/20 rounded-lg bg-white shadow-sm">
               ...
               <div className="grid md:grid-cols-3 gap-4">
                 ...
                 <div>
                   <label className="block mb-2">Heure de retour</label>
                   <input
                     type="time"
  ...
  {additionalInfo.paymentMethod === 'invoice' && (
    <div className="mt-4">
      ...
      <input
        ...
        onChange={handleAdditionalInfoChange}
        className={`w-full p-3 border rounded-md ${formErrors.vatNumber ? 'border-red-500' : ''}`}
        placeholder="BE0123456789"
        required
      />
      {formErrors.vatNumber && (
        <p className="text-red-500 text-sm mt-1">{formErrors.vatNumber}</p>
      )}
    </div>
  )}
</div>
