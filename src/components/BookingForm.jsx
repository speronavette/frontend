...
// Constantes séparées du composant principal
const AIRPORTS = [
...
// Données structurées pour le formulaire (memoized pour éviter les recréations)
const getBookingFormData = () => ({
...
function BookingForm() {
 ...
 // Grouper les états liés dans des objets pour réduire le nombre de re-rendus
 ...
 // Memoize ces données pour éviter les recalculs inutiles
 const bookingFormData = useMemo(() => getBookingFormData(), []);
 
 // Verification de la validité du prix en fonction des données
 const isPriceValid = useMemo(() => {
   ...
   switch (journeyType) {
     case 'outbound':
       return checkPrice(postalCode, outboundJourney.airport);
       
     case 'inbound':
       return checkPrice(postalCode, inboundJourney.airport);
       
     case 'roundTrip':
       // Si les deux aéroports ne sont pas encore sélectionnés, on retourne true
       if (!outboundJourney.airport || !inboundJourney.airport) return true;
       
       // Sinon on vérifie que les deux ont des prix valides
       return checkPrice(postalCode, outboundJourney.airport) && 
              checkPrice(postalCode, inboundJourney.airport);
 ...
 // Mettre à jour le prix quand les données pertinentes changent
 useEffect(() => {
   const { postalCode, passengers } = personalInfo;
 ...
 const handlePostalCodeChange = (e) => {
   ...
   setPersonalInfo(prev => ({
     ...prev,
     postalCode: code,
     city: code.length === 4 ? (postalCodesDB[code] || 'Code postal non trouvé') : ''
   }));
 };
 ...
 const handlePersonalInfoChange = (e) => {
   ...
   // Validation en temps réel - utiliser un debounce si nécessaire
 ...
 const handleOutboundChange = (e) => {
   const { name, value } = e.target;
   setOutboundJourney(prev => ({ ...prev, [name]: value }));
 };

 const handleInboundChange = (e) => {
   const { name, value } = e.target;
   setInboundJourney(prev => ({ ...prev, [name]: value }));
 };
 ...
 // Fonction optimisée pour construire les données de réservation
 const buildBookingData = () => {
   // Base commune pour tous les types de réservation
   const commonData = {
     client: {
   ...
   // Données spécifiques au type de voyage
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    // Au lieu d'appeler l'API, utiliser la fonction mockée
    const result = await window.handleFormSubmit(bookingData);
 ...
 // Calculer le titre et la description de la page dynamiquement
 ...
 // Calcul du prix total courant (pour le bouton de soumission)
 const currentPrice = useMemo(() => {
   return serviceType === 'private' ? price.privatePrice : price.sharedPrice;
 }, [serviceType, price]);
       ...
       <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-xl p-8 space-y-8">
         ...
         <section>
           ...
           <div className="grid md:grid-cols-2 gap-6">
             ...
             {/* Code postal, commune et localité */}
         ...
         <div className="mt-8 flex justify-center">
           <button
             type="submit"
             disabled={isSubmitting || currentPrice === 0 || !termsAccepted}
             className={`px-8 py-3 rounded-md transition-colors ${
               isSubmitting || currentPrice === 0 || !termsAccepted
